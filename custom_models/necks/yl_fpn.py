...
class YeFPNv2(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_cfg, num_stack=1, num_output=5, start_level=0):
        """
        Args:
            num_channels:
            conv_channels:
    ...
    def forward(self, x):
        ...
        if self.num_extra:
            ...
            # tmp = x[-1]
            # for i in range(self.num_extra):
            #     tmp = self.extra_convs[i](tmp)
            #     extra.append(tmp)
            # P3, P4, P5, P6, P7 = x[self.start_level:]+tuple(extra)
        else:
            P3, P4, P5, P6, P7 = x
